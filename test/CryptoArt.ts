...
describe("CryptoartNFT", function () {
  ...
  describe("Minting", function () {
    ...
    it("Should revert operations on non-existent tokens", async function () {
      await expect(cryptoArtNFT.tokenURI(_tokenId1)).to.be.revertedWith(
  ...
  describe("Metadata", function () {
    ...
    it("Should revert when updating metadata for non-existent token", async function () {
      ...
      const newURI = "https://newuri.com/token/999";

      await expect(
        cryptoArtNFT.connect(_owner).updateMetadata(nonExistentTokenId, newURI),
      ).to.be.revertedWith("Token does not exist");
    });

    it("Should emit MetadataUpdate event when updating metadata", async function () {
      // First mint a token
      const { id, signature } = await getSignatureForMint(
        cryptoArtNFT,
        addr1.address,
      ...
      const newURI = "https://newuri.com/token/1";

      await expect(cryptoArtNFT.connect(_owner).updateMetadata(id, newURI))
        .to.emit(cryptoArtNFT, "MetadataUpdate")
        .withArgs(id);
    });
  });
