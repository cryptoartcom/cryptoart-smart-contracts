...
describe("Reentrancy Protection Tests", function () {
  ...
  it("Should prevent reentrancy attacks when contract is paused", async function () {
    const tokenId1 = 1;
    const mintPrice = ethers.parseEther("0.01");
    const maliciousReceiverAddress = await maliciousReceiver.getAddress();
    ...
    // Pause contract
    await cryptoArtNFT.connect(owner).pause();

    // Setup and attempt various minting functions
    await maliciousReceiver.setMintParams(
      tokenId1,
      MintTypesEnum.OpenMint,
      mintPrice,
